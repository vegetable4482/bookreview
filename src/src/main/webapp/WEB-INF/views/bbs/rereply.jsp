...
<script>
	...
	//대댓글 작성 html코드 시작ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
	var rereply_str ="";
	...
	rereply_str += '    <a class="rrreply badge badge-pill badge-light mr-3">댓글</a> ';
	rereply_str += ' </div> ';
	rereply_str += '</div> ';
	
	//대댓글 작성 html코드 끝ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
	
	//대댓글 수정 html코드 시작 ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
	var replyModify_str ="";
	...
	replyModify_str += '     <a class="rrmodify badge badge-pill badge-light mr-3">수정</a> ';
	replyModify_str += ' </div> ';
	replyModify_str += '</div> ';
	//대댓글 수정 html코드 끝ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
	...
	$(function(){
		...
		function replyList(reqPage){
			...
			$.ajax({
				...
				success:function(result){		
					$.each(result.item,function(idx, rec){
						...
						if(rec.rindent == 0 ){
							...
							str += '			<div class="row m-2 p-0"> ';
							str += '				<div class="col-1"> ';
							str += '					<img src="${pageContext.request.contextPath }/resources/img/icon_person.JPG" ';
							str += '							 class="rounded-cicle" style="width:45px;"  ';
							str += '							 alt="" /> ';
							str += '				</div> ';
							str += '				<div class="col-10"> ';
							str += '					<h6> '+rec.rnickname+'('+rec.rid+') <small class="ml-2">'+rec.rcdate+'</small></h6> ';
							str += '					<p></span>'+rec.rcontent+'</p> ';
							str += '				</div> ';
							str += '			</div>	 ';
							...
							str += '					<a class="rereplyBtn badge badge-pill badge-light">댓글</a> ';
							str += '				</div>		 ';
							...
							str += '				<div class="col-8"> ';
							str += '					<h6> '+rec.rnickname+'('+rec.rid+') <small class="ml-2">'+rec.rcdate+'</small></h6> ';
							str += '					<p><span class="mr-1 text-warning ">'+rec.rrnickname+'</span>'+rec.rcontent+'</p> ';
							str += '				</div> ';
							str += '			</div>	 ';
							str += '			<div class="row m-0 p-0"> ';
						...
						//댓글작성자 아이디와 로긴 아이디가 동일한 경우만 보이도록함.
						if(l_id == rec.rid ){
				...
				//페이지번호 클릭시 이벤트 처리
				//id값이 paging인 하위 요소 li a 에 클릭이벤트가 발생하면 
				$("#paging").on("click","li a", function(e){
					//현재 이벤트의 기본동작을 중단
					e.preventDefault();

					//현재 이벤트가 상위 및 현재 레벨에 거린 다른 이벤트도 동작 않도록 중단
  ...
  <div class="row justify-content-end my-1">
		<div class="col-sm-1 px-0 m-1">
     	<button class="btn btn-sm btn-outline-secondary btn-block" id="replyCancelBtn">취소</button>
    </div>
		<div class="col-sm-1 px-0 m-1">
	...
	<div class="row">
		<div id="replyList" class="col m-0 p-0">
			...
			<div class="row m-2 p-0">
				<div class="col-1">
					<img src="${pageContext.request.contextPath }/resources/img/icon_person.JPG"
							 class="rounded-circle" style="width:45px" 
							 alt="" />
				</div>
				<div class="col-11">
					<h6>별칭(아이디) <small class="ml-2">2019-09-17</small></h6>
					<p><span class="mr-1 text-warning ">부모댓글별칭</span>댓글내용</p>
				</div>
			</div>	
			<div class="row m-0 p-0">
				...
				<div class="col-8">
					...
					<a class="badge badge-pill badge-light">댓글</a>
				</div>		
